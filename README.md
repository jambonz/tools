...
Call `listTools()` to enumerate every tool this version of the package ships —
without importing or instantiating any of them. Useful for tooling, docs, and AI
coding agents deciding whether a ready-made tool already covers a need before
hand-rolling one:

```typescript
import { listTools } from '@jambonz/tools';

for (const t of listTools()) {
  console.log(`${t.name} — ${t.description}`);
  console.log(`  import { ${t.factory} }${t.requiresApiKey ? ' (needs API key)' : ''}`);
}
```

Each entry is a `ToolInfo`: `{ name, description, factory, requiresApiKey, parameters }`.
The catalog is generated by globbing `src/tools/`, so it always reflects the
installed version and never needs manual maintenance.
...
1. Create `src/tools/your-tool.ts` and export three things:
...
2. Add the factory export to `src/index.ts`
3. Add documentation to this README

The tool catalog (`listTools()`) regenerates automatically on build and
pre-commit — there is no list to update by hand.
